...
def get_context(
    ...
    for score, idx in zip(scores[0], indices[0]):
        ...
        if subject_filter:
            # Map DB subject codes to knowledge-file subject codes.
            # e.g. Subject model uses 'AI' but knowledge files are labelled 'AIML'.
            _SUBJECT_ALIASES = {'AI': 'AIML'}
            chunk_subj = chunk.get("subject", "").upper()
            filter_code = _SUBJECT_ALIASES.get(subject_filter.upper(), subject_filter.upper())
            # Exact match only — startswith caused 'DS' to wrongly match 'DSC' chunks.
            if chunk_subj and chunk_subj != filter_code:
                continue
